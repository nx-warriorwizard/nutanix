    ...
    if resp.ok :
        ...
        for vpc_data in result['data']:
            if vpc_data["name"] == vpc_name:
                ...
                print("[INFO] Vpc Etag is : ", vpc_etag)
                if not(resp.ok):
...
def update_route(api_server, username, passwd, headers, vpc_extId, route_table_extId, subnet_uuid, subnet_name, secure):
    #check if route available if present skip  #### not using as process monitor requires uuid in return
    ...
    if total_available_route > 0:
        print("[INFO] Route already exists, skipping.")
    #     task_uuid = route_data['data']['extId']
    #     return task_uuid
    url = f"https://{api_server}:9440/api/networking/v4.0/config/route-tables/{route_table_extId}/routes"
    payload = {
        "routeType": "STATIC",
...
def prism_flow_update_vpc(api_server, username, passwd, headers, vpc_name, subnet_uuid, subnet_name, secure=False):
    ...
    print("[INFO] Older Subnet UUID:", old_uuid)
    if subnet_uuid == old_uuid:
        ...
        print(f"[INFO] VPC update status {resp.status_code} : {resp.content}")

        print("[INFO] Route adding in progress ")
        task_uuid = update_route(api_server, username, passwd, headers, vpc_extId, route_table_extId, subnet_uuid, subnet_name, secure=secure)
